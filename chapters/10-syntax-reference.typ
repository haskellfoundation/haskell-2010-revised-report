...
- A stream of lexemes as specified by the lexical syntax in the Haskell report, with the following additional tokens: 
  ...
  - Where the start of a lexeme is preceded only by white space on the
    same line, this lexeme is preceded by $nontermname("<n>")$ where $n$
    ...
    f = ("Hello \
          \Bill", "Jake")
    ```
    There is no $nontermname("<n>")$ inserted before the `\Bill`, because it is not the beginning of
...
- The characters $nonterminal("newline")$, $nonterminal("return")$, $nonterminal("linefeed")$, and $nonterminal("formfeed")$, all start a new line.
...
L tokens []
```
delivers a layout-insensitive translation of $varname("tokens")$, where $varname("tokens")$
...
/ Note 5.: The side condition $varname("parse-error") (t)$ is to be interpreted as follows:
  ...
  tokens generated so far by $L$ followed by the token "`}`" represent a valid prefix of the Haskell grammar, then $varname("parse-error")(t)$ is true.
...
#table(
  ...
  // constr
  $nonterminaldef("constr")$, $->$, $nonterminal("con") [terminal("!")] nonterminal("atype")_1 dots [terminal("!")] nonterminal("atype")_k$, [(arity $nonterminal("con") = k$, $k >= 0$)],
...
expressions or operators, i.e. an instance of the $nonterminal("infixexp")$ non-terminal in the context-free grammar.  It returns either `Just e`
